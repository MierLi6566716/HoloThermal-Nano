...
from utils.fft_compat import fft_compat, ifft_compat
...
def ifft2(tensor_re, tensor_im, shift=False):
    ...
    (tensor_out_re, tensor_out_im) = ifft_compat(tensor_out, 2, True).split(1, 4)
...
def fft2(tensor_re, tensor_im, shift=False):
    ...
    # fft2
    (tensor_out_re, tensor_out_im) = fft_compat(torch.stack((tensor_re, tensor_im), 4), 2, True).split(1, 4)
