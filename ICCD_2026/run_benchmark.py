...
from plotting.plot_comprehensive import plot_comprehensive
...
def ensure_results_dirs():
    for path in ["results", "results/csv", "results/plots"]:
...
def prepare_nonbaseline_mode(config):
    if config["scheme_name"] == "baseline":
        print("Baseline run: ensuring system thermal throttling is ENABLED.")
        dvfs.enable_thermal_throttling()
        return
...
def main():
    ...
    csv_path = os.path.join("results", "csv", stem + ".csv")
    plot_path = os.path.join("results", "plots", stem + ".png")
    ...
    peak_c, average_c, average_p_mw = temp_utils.compute_csv_stats(csv_path)
    elapsed_s = wall_end - wall_start

    from utils.power_utils import compute_energy_joules
    total_energy_j = compute_energy_joules(average_p_mw, elapsed_s)

    plot_comprehensive(csv_path=csv_path, output_path=plot_path, title_stem=stem)

    with open(os.path.join("results", "summary.txt"), "a") as summary_file:
        summary_file.write(
            "scheme=%s, stress=%s, start_temp_c=%.2f, baseline_temp_c=%.2f, peak_temp_c=%.2f, average_temp_c=%.2f, average_p_mw=%.2f, total_energy_j=%.2f, elapsed_s=%.2f\n"
            % (
                config["scheme_name"],
