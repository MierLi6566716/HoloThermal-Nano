...
def plot_comprehensive(csv_path, output_path, title_stem):
    """
    Renders a multi-panel plot (Temp, Freq, Power, Energy) for a single CSV.
    Logic mirrored from plot_comprehensive.py.
    """
    try:
...
def main():
    ...
    print(f"Found {len(csv_files)} CSV files. Generating comprehensive plots...")
    
    for csv_path in sorted(csv_files):
        base_name = os.path.basename(csv_path).replace(".csv", "")
        output_path = os.path.join(PLOT_DIR, f"comprehensive_{base_name}.png")
        
        print(f"  Plotting {base_name}...")
        plot_comprehensive(csv_path, output_path, base_name)

    print(f"\nSuccess! All plots saved to: {PLOT_DIR}")
