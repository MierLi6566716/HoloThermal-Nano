...
def _patched_create_model(model_name, **kwargs):
    if model_name == 'vit_tiny_patch16_224':
        try:
            ...
            img_size = (256, 144) # REDUCED RESOLUTION
    ...
    if 'agent' not in _AGENT_CACHE:
        class Args:
            def __init__(self):
                ...
                self.w_mse = 1.0; self.w_vgg = 0.025; self.w_ssim = 0.05; self.w_wfft = 1e-8
                self.nr_epochs = 1; self.lr = 1e-3; self.lr_s = 5e-5; self.lr_step_size = 5
                self.cont = False; self.vis = False; self.save_frequency = 1; self.val_frequency = 5
...
def _background_output_loop():
    ...
    while True:
        ...
        recon_img, base_input, out_path, scheme, i, unit_duration, fps, num_threads, os_pid = task
        ...
        print(f"[{os_pid}] Unit {i}: {unit_duration:.2f}s ({fps:.2f} FPS) | SSIM={score:.4f} | Threads={num_threads}")
...
def run_units(units, config_thermal):
    ...
    print(f"[{os.getpid()}] Running {units} units ({scheme}) [INTRA-OP]... Threads: {num_threads}")
    
    # In Intra-Op mode, we process 1 image using N threads
    for i in range(int(units)):
        ...
        with torch.no_grad():
            ...
            if i % 20 == 0:
                ...
                data_for_bg = (recon_img.detach().clone(), single_input, out_path, scheme, i, unit_duration, fps, num_threads, os.getpid())
                _output_queue.put(data_for_bg)
            elif i % 5 == 0:
                print(f"[{os.getpid()}] Unit {i}: {unit_duration:.2f}s ({fps:.2f} FPS) | Threads={num_threads}")
