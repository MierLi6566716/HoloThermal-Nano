...
def get_dataloaders(
    ...
    **kwargs
    ...
    if root is None:
        ...
        dataset = Dataset(
            root=root, logger=logger, mode=mode, normalize=normalize, **kwargs
        )
...
class OpenImages(BaseDataset):
    ...
    def __init__(
        self,
        root=os.path.join(DIR, "data/openimages"),
        mode="train",
        crop_size=256,
        normalize=False,
        **kwargs
    ...
    def __init__(self, mode, root=os.path.join(DIR, "data/cityscapes"), **kwargs):
        super().__init__(
            ...
            transform=self._transforms(
                scale=np.random.uniform(0.5, 1.0), H=512, W=1024
            ),
        )
