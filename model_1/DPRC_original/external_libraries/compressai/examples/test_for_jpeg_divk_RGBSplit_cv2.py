...
for quality in [90, 85, 80]:
    print(f"{quality}")
    writer.write(
        f"_____________________________quality{quality}_________________________________\n"
    )
    all_time_enc = [[], [], []]
    all_time_dec = [[], [], []]
    all_bpp = [[], [], []]
    ...
    for i in range(801, 901):
        img_name = os.path.join(
            f"/jixie/yujie_data/DIV2K_valid_HR/{i:04d}-gt-target.png"
        )
        ...
        for m, img in enumerate([red, green, blue]):
            ...
            count += 1
            img_np = np.array(img)
            save_name = str.replace(
                img_name, ".png", f"-{channel_strs[m]}-quality{quality}.jpg"
            )
            print(f"m is {m}  ", save_name)
            cv2.imwrite(save_name, img_np, [cv2.IMWRITE_JPEG_QUALITY, quality])
            output = {}
            output["bpp"] = (
                os.path.getsize(save_name) * 8 / (img_np.shape[0] * img_np.shape[1])
            )
