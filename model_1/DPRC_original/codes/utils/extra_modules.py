...
class ResidualBlock(nn.Module):
    def __init__(
        self, in_channels, kernel_size=3, stride=1, channel_norm=True, activation="relu"
    ):
        """
    ...
    def __init__(
        ...
        for m in range(n_residual_blocks):
            resblock_m = ResidualBlock(
                in_channels=filters[0], channel_norm=False, activation=activation
            )
        ...
        for i in range(0, len(filters) - 1):
            cur_block = nn.Sequential(
                nn.ConvTranspose2d(
                    filters[i], filters[i + 1], kernel_dim, **cnn_kwargs
                ),
...
class MultiScale_Encoder(nn.Module):
    ...
    def __init__(
        ...
        if not len(per_layer_out_ch):
            ...
            self.direct_down1 = nn.Sequential(
            ...
            self.direct_down2 = nn.Sequential(
                nn.ReflectionPad2d(0),
                nn.Conv2d(
                    in_channels,
                    per_layer_out_ch[-1],
        ...
        if out_channels != None:
            out_layer = Conv2dSame(
                int(per_layer_out_ch[-1]), out_channels, kernel_size=3, bias=True
            )
