...
def ifft2(tensor_re, tensor_im, shift=False):
    ...
    (tensor_out_re, tensor_out_im) = my_ifft(tensor_out, 2, True).split(1, 4)
...
def fft2(tensor_re, tensor_im, shift=False):
    ...
    # fft2
    (tensor_out_re, tensor_out_im) = my_fft(
...
def write_sgd_summary(
    ...
    psnr_value = psnr(
        target_amp.cpu().detach().numpy(), (s * out_amp).cpu().detach().numpy()
    )
    ssim_value = ssim(
        target_amp.cpu().detach().numpy(),
        (s * out_amp).cpu().detach().numpy(),
        multichannel=True,
    )

    if writer is not None:
        writer.add_image(
            f"{prefix}_Recon/amp", (s * out_amp).squeeze(0), k, dataformats="HWC"
        )
