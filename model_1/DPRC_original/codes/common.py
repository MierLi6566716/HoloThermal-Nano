...
class Config(object):
    ...
    def __init__(self, phase):
        ...
        wave_dict = {
            ...
            "g": 520 * nm,
            "r": 638 * nm,
        }  # pre-defined wavelength for each color channel

        self.chan_strs = ("red", "green", "blue")
        self.prop_dist = self.prop_dist * cm
        self.feature_size = (args.pixel_pitch * um, args.pixel_pitch * um)
        self.wavelength = wave_dict[
            self.channel
        ]  # select the wavelength used for wave propagation calculation
        ...
        self.target_schedule = dict(
            vals=[0.20 / 0.14, 1.0], steps=[10000]
        )  # Rate allowance
        ...
        if self.model_name == "":
            ...
            self.model_dir = os.path.join(
                self.exp_dir, "model_{}".format(self.model_name)
            )
        ...
        )  # remove shutil.retree to avoid accidentally removing ckpts!!!
        os.makedirs(self.model_dir, exist_ok=True)

        # GPU usage
        if args.gpu_ids is not None:
            self.device = f"cuda:{args.gpu_ids}"
        else:
    ...
    def _add_basic_config_(self, parser):
        ...
        group.add_argument(
            "--holo_data_root", type=str, default="/data1/test/tianlong/huaizhi/data"
        )
        group.add_argument(
            "--exp_name",
            type=str,
    ...
    def _add_dataset_config_(self, parser):
        ...
        group.add_argument(
            ...
            type=str,
            choices=["r", "g", "b"],
            help="select which channel to train",
        )
        group.add_argument(
            "--fake",
            action="store_true",
    ...
    def _add_display_config_(self, parser):
        ...
        group.add_argument(
            "--prop_dist", type=int, default=20, help="the propagation distance in cm"
        )
        group.add_argument(
            "--pixel_pitch", type=float, default=6.4, help="pixel pitch for SLM"
        )
    ...
    def _add_network_config_(self, parser):
        ...
        group.add_argument(
            ...
            action="store_true",
            default=False,
            help="whether to integrate compression modules into the framework",
        )
    ...
    def _add_loss_config_(self, parser):
        ...
        group.add_argument(
            ...
            type=float,
            default=1.0,
            help="the weight parameter for MSE loss term",
        )
        group.add_argument(
            "--w_vgg",
            type=float,
            default=0.025,
            help="the balancing weight for VGG perceptual loss",
        )
        group.add_argument(
            "--w_ssim",
            type=float,
            default=0.05,
            help=" the balancing weight for MS-SSIM loss",
        )
        group.add_argument(
            "--w_wfft",
            type=float,
            default=1e-8,
            help="the balancing weihgt for Watson-FFT loss",
        )
        group.add_argument(
            "--quality",
            type=str,
            ...
            help="the compression quality level set for stage2's training",
        )
    ...
    def _add_training_config_(self, parser):
        ...
        group.add_argument(
            "--lr_s", type=float, default=5e-5, help="initial learning rate"
        )
        group.add_argument(
            "--lr_step_size",
            type=int,
        ...
        group.add_argument(
            "--save_frequency", type=int, default=1, help="save models every x epochs"
        )
        group.add_argument(
            "--val_frequency",
            type=int,
        ...
        group.add_argument(
            ...
            type=str,
            default="",
            help="specify a mdoel name for save the model and log",
        )
    ...
    def _add_test_config_(self, parser):
        ...
        group.add_argument(
            "-o", "--output", type=str, default="output folder to save results"
        )
        group.add_argument(
            "--postfix", type=str, default="", help="postfix for name the output"
        )
