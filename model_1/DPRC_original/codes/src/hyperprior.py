...
from src.compression import compression_utils, hyperprior_model, prior_model
...
class CodingModel(nn.Module):
    ...
    def __init__(
        self, n_channels, min_likelihood=MIN_LIKELIHOOD, max_likelihood=MAX_LIKELIHOOD
    ):
...
class Hyperprior(CodingModel):
    ...
    def __init__(
        ...
        self.synthesis_stats = synthesis_net(
            C=int(2 * bottleneck_capacity), N=hyperlatent_filters
        )

        self.amortization_models = [self.analysis_net, self.synthesis_stats]

        self.hyperlatent_likelihood = hyperprior_model.HyperpriorDensity(
            n_channels=hyperlatent_filters
        )
        ...
        if entropy_code is True:
            ...
            self.prior_density = prior_model.PriorDensity(
                n_channels=int(bottleneck_capacity * 16),
    ...
    def reshape(self, input_tensor):
        ...
        input_tensor = (
            input_tensor.permute(0, 1, 2, 4, 3, 5)
            .contiguous()
            .view(B, int(C * 16), H // 4, W // 4)
        )
        return input_tensor

    def reshape_back(self, input_tensor):
        B, C, H, W = input_tensor.shape
        input_tensor = input_tensor.view(B, C // 16, 16, H, W).view(
            B, C // 16, 4, 4, H, W
        )
        input_tensor = (
            input_tensor.permute(0, 1, 2, 4, 3, 5)
            .contiguous()
            .view(B, C // 16, int(H * 4), int(W * 4))
        )
        return input_tensor
    ...
    def compress_forward(self, latents, spatial_shape, **kwargs):
        ...
        hyperlatents_encoded, hyper_coding_shape, _ = (
        ...
        latent_means, latent_scales = torch.split(
            latent_stats, self.bottleneck_capacity, dim=1
        )
        ...
        latents_encoded, latent_coding_shape, _ = self.prior_entropy_model.compress(
            latents,
            means=latent_means,
        ...
        latent_agg = self.prior_entropy_model._estimate_compression_bits(
            latents,
            means=latent_means,
            scales=latent_scales,
    ...
    def decompress_forward(self, compression_output, device):
        ...
        hyperlatents_decoded = self.hyperprior_entropy_model.decompress(
            hyperlatents_encoded,
            batch_shape=batch_shape,
        ...
        latents_decoded = self.prior_entropy_model.decompress(
            latents_encoded,
            means=latent_means,
            scales=latent_scales,
    ...
    def forward(self, latents, spatial_shape, **kwargs):
        ...
        quantized_hyperlatents = self._quantize(hyperlatents, mode="quantize")
        quantized_hyperlatent_likelihood = self.hyperlatent_likelihood(
        ...
        noisy_latent_likelihood = self.latent_likelihood(
            noisy_latents, mean=latent_means, scale=latent_scales
        )
        ...
        quantized_latent_likelihood = self.latent_likelihood(
            quantized_latents, mean=latent_means, scale=latent_scales
        )
...
class Hyperprior_new(CodingModel):
    ...
    def __init__(
        ...
        if entropy_code is True:
            ...
            self.prior_density = prior_model.PriorDensity(
                n_channels=bottleneck_capacity,
                scale_lower_bound=self.scale_lower_bound,
                likelihood_type=likelihood_type,
            )
            self.prior_entropy_model = prior_model.PriorEntropyModel(
    ...
    def decompress_forward(self, compression_output, device):
        ...
        # Use latent statistics to build indexed probability tables, and decompress latents
        latents_decoded, _ = self.prior_entropy_model.decompress(
            latents_encoded,
            means=latent_means,
            scales=latent_scales,
            broadcast_shape=latent_spatial_shape,
...
class HyperpriorDLMM(CodingModel):
    ...
    def __init__(
        ...
        self.synthesis_DLMM_params = synthesis_net(
            C=bottleneck_capacity, N=hyperlatent_filters
        )
    ...
    def latent_log_likelihood_DLMM(self, x, DLMM_params):
        ...
        # (B C K H W)
        x, (logit_pis, means, log_scales), K = hyper.unpack_likelihood_params(
            x, DLMM_params, LOG_SCALES_MIN
        )
        ...
        pmf_mixture_component = lower_bound_toward(
            cdf_upper - cdf_lower, MIN_LIKELIHOOD
        )
    ...
    def forward(self, latents, spatial_shape, **kwargs):
        ...
        noisy_latent_log_likelihood = self.latent_log_likelihood_DLMM(
            noisy_latents, DLMM_params=latent_DLMM_params
        )
        ...
        quantized_latent_log_likelihood = self.latent_log_likelihood_DLMM(
            quantized_latents, DLMM_params=latent_DLMM_params
        )
