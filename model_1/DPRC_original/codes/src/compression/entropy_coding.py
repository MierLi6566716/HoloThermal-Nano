...
def base_codec(enc_statfun, dec_statfun, precision, log=False):
    if np.any(precision >= 24):
        warn("Detected precision over 28. Codecs lose accuracy at high " "precision.")
...
def _vec_indexed_cdf_to_enc_statfun(cdf_i):
    ...
    def _enc_statfun(value):
        ...
        upper = np.take_along_axis(cdf_i, np.expand_dims(value + 1, -1), axis=-1)[
            ..., 0
        ]
...
def _indexed_cdf_to_dec_statfun(cdf_i, cdf_i_length):
    ...
    term = cdf_i[-1]
    assert (
        term == OVERFLOW_CODE or term == 1 << OVERFLOW_WIDTH
...
def ans_index_buffered_encoder(
    symbols,
    indices,
    cdf,
    cdf_length,
    ...
    **kwargs,
    ...
    overflow_push, overflow_pop = base_codec(
        enc_statfun_overflow, dec_statfun_overflow, overflow_width
    )
    ...
    for i in range(len(indices)):  # loop over flattened axis
        ...
        assert (
            cdf_index >= 0 and cdf_index < cdf.shape[0]
        ), f"Invalid index {cdf_index} for symbol {i}"

        max_value = cdf_length_i - 2

        assert (
            max_value >= 0 and max_value < cdf.shape[1] - 1
...
def ans_index_encoder_flush(
    instructions, precision, overflow_width=OVERFLOW_WIDTH, **kwargs
):
...
def vec_ans_index_buffered_encoder(
    ...
    **kwargs,
    ...
    assert bool(
        np.all(values < cdf_length[cdf_index] - 1)
    ...
    if B == 1:
        ...
        if (
            (symbols_shape[2] % patch_size[0] == 0)
            and (symbols_shape[3] % patch_size[1] == 0)
        ) is False:
            values = (
                utils.pad_factor(
                    torch.Tensor(values), symbols_shape[2:], factor=patch_size
                )
                .cpu()
                .numpy()
                .astype(np.int32)
            ...
            indices = (
                utils.pad_factor(
                    torch.Tensor(indices), symbols_shape[2:], factor=patch_size
                )
                .cpu()
                .numpy()
                .astype(np.int32)
            ...
            overflow = (
                utils.pad_factor(
                    torch.Tensor(overflow), symbols_shape[2:], factor=patch_size
                )
                .cpu()
                .numpy()
                .astype(np.int32)
        ...
        assert (values.shape[2] % patch_size[0] == 0) and (
            values.shape[3] % patch_size[1] == 0
        )
        assert (indices.shape[2] % patch_size[0] == 0) and (
            indices.shape[3] % patch_size[1] == 0
        )
...
def vec_ans_index_encoder_flush(
    instructions, precision, coding_shape, overflow_width=OVERFLOW_WIDTH, **kwargs
):
...
def vec_ans_index_encoder(
    ...
    **kwargs,
    ...
    start = time.time()
    encoded = vec_ans_index_encoder_flush(
        instructions, precision, coding_shape, overflow_width
    )
    # print('second step elapsed time ... ', time.time() - start)
    return encoded, coding_shape


def ans_index_decoder(
    encoded,
    indices,
    cdf,
    cdf_length,
...
def vec_ans_index_decoder(
    ...
    **kwargs,
    ...
    if B == 1:
        ...
        if (
            (original_shape[2] % patch_size[0] == 0)
            and (original_shape[3] % patch_size[1] == 0)
        ) is False:
            indices = (
                utils.pad_factor(
                    torch.Tensor(indices), original_shape[2:], factor=patch_size
                )
                .cpu()
                .numpy()
                .astype(np.int32)
    ...
    if B == 1:
        decoded = compression_utils.reconstitute(
            np.stack(symbols, axis=0),
            padded_shape,
            unfolded_shape,
            patch_size=patch_size,
        )
