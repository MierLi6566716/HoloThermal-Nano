...
def estimate_tails(cdf, target, shape, dtype=torch.float32, extra_counts=24):
    ...
    while torch.min(counts) < extra_counts:
        ...
        with torch.no_grad():
            m = beta_1 * m + (1.0 - beta_1) * tgrad
            v = beta_2 * v + (1.0 - beta_2) * torch.square(tgrad)
...
def view_update(data, view_fun):
    ...
    def update(new_item):
        assert item_vs == vspace(
            new_item
        ), "Please ensure new_item shape and dtype match the data view."
        diff = view_vjp(item_vs.add(new_item, item_vs.scalar_mul(item, -np.uint64(1))))
...
def decompose(x, n_channels, patch_size=PATCH_SIZE):
    ...
    y = (
        x.unfold(1, n_channels, n_channels)
        .unfold(2, *patch_size)
        .unfold(3, *patch_size)
    )
    unfolded_shape = tuple(y.size())
    y = torch.reshape(
        y, (-1, n_channels, *patch_size)
    )  # (n_patches, n_channels, *patch_size)
...
def reconstitute(x, original_shape, unfolded_shape, patch_size=PATCH_SIZE):
    ...
    x_re = torch.reshape(
        x, (B, 1, unfolded_shape[2], unfolded_shape[3], n_channels, *patch_size)
    )
...
def check_argument_shapes(cdf, cdf_length, cdf_offset):
    ...
    if len(cdf_length.size()) != 1 or cdf_length.size(0) != cdf.size(0):
        raise ValueError(
            "'cdf_length' should be 1-D and its length "
    ...
    if len(cdf_offset.size()) != 1 or cdf_offset.size(0) != cdf.size(0):
        raise ValueError(
            "'cdf_offset' should be 1-D and its length "
            "should match the number of rows in 'cdf': ",
            cdf_offset.size(),
        )
...
def save_compressed_format(compression_output, out_path):
    ...
    actual_bpp = (
        8.0 * float(actual_num_bytes) / np.prod(compression_output.spatial_shape)
    )
    try:
...
if __name__ == "__main__":
    ...
    norm_cdf = lambda x: 0.5 * (1.0 + torch.erf(x / math.sqrt(2)))
