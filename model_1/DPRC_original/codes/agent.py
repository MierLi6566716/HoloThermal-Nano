...
class HologramAgent(BaseAgent):
    ...
    def set_loss_function(self):
        self.criterionMSE = nn.MSELoss(size_average=True).to(
            self.device
        )  # mse loss criterion
        ...
        self.criterionWFFT = provider.get_loss_function(
            "watson-fft",
            deterministic=False,
            colorspace="grey",
            pretrained=True,
            reduction="sum",
        )  # watson-fft loss criterion
        ...
        self.propagator = holo_propagator(
            self.config.wavelength, self.config.prop_dist, self.config.feature_size
        )
    ...
    def scale(self, x, label, weight=None):
        ...
        if weight is not None:
            ...
            label = torch.mul(
                torch.autograd.Variable(weight, requires_grad=False), label
            )
    ...
    def forward(self, data):
        ...
        if not self.config.compress:
            ...
            if self.config.is_train:
                ...
                weighted_rate, rate_penalty = losses.weighted_rate_loss(
                    self.config,
                    total_nbpp=total_nbpp,
                    total_qbpp=total_qbpp,
                    step_counter=self.clock.step,
                    ignore_schedule=False,
                )
                rate_loss = weighted_rate.to(
                    "cuda:0"
                )  # this is the rate loss used for lowering the bit-rate
                total_qbpp = (
                    total_qbpp.detach()
                )  # just for evaluation and curve visualization, with no gradient back-propagation
                total_nbpp = (
                    total_nbpp.detach()
                )  # just for evaluation and curve visualization, with no gradient back-propgation
                loss_dict.update(
                    {
                        "rate_loss": rate_loss,
                        "eval_bpp": total_qbpp,
                        "eval_nbpp": total_nbpp,
                    }
                )
            else:
                output = self.net(target_amp, self.config.is_train)
        ...
        if not self.config.is_train:
            ...
            vgg_loss = self.config.w_vgg * self.criterionVGG_recon(
                recon_img_scale, target_amp
            )

        msssim_loss = self.config.w_ssim * (
            1 - self.criterionMS_SSIM(recon_img_scale, target_amp, data_range=1)
        )

        recon_losses = {
            "watson-fft": wfft_loss,
            "mse": mse_loss,
            "ms_ssim": msssim_loss,
            "vgg": vgg_loss,
        }
    ...
    def visualize_batch(self, data, mode, outputs=None):
        ...
        target_recon = F.interpolate(target_recon.unsqueeze(0), scale_factor=0.25)[
            0, ...
        ]
        ...
        tb.add_image(
            "recon_img_scale", recon_img_scale, self.clock.step, dataformats="CHW"
        )
