...
def get_network(config):
    ...
    hyper_prior = hyperprior.Hyperprior(bottleneck_capacity=8, likelihood_type='gaussian', entropy_code=entropy_code)
...
class TransEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = timm.create_model('vit_tiny_patch16_224', pretrained=False, num_classes=0, global_pool="",
                                          img_size=(1344, 1792), in_chans=2, features_only=True, embed_dim=96)
        self.upscale = nn.Sequential(
            nn.ConvTranspose2d(96, 12, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), output_padding=(1, 1)),
            nn.BatchNorm2d(12, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
            nn.ReLU(),
            nn.ConvTranspose2d(12, 8, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), output_padding=(1, 1)),
...
class InitialPhaseUnet(nn.Module):
    ...
    def __init__(
        self, num_down=8, num_features_init=32, max_features=256, norm=nn.BatchNorm2d
    ):
        super(InitialPhaseUnet, self).__init__()

        net = [Unet(1, 1, num_features_init, num_down, max_features, use_dropout=False, upsampling_mode='transpose',
                    norm=norm, outermost_linear=True),
               nn.Hardtanh(-math.pi, math.pi)]
        # net = [SwinUnet(), nn.Hardtanh(-math.pi, math.pi)]
...
class ComPhaseNet(nn.Module):
    def __init__(
        ...
        self.decoder = Generator(filters=[64, 32, 16], in_channels=8, activation='relu', n_residual_blocks=4)
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.feature_size = (
            feature_size if hasattr(feature_size, "__len__") else [feature_size] * 2
        )
    ...
    def forward(self, target_amp, is_train=True):
        ...
        if not self.is_stage2:
            ...
            if is_train:
                ...
                compression_output = self.hyper_prior.compress_forward(
                    latent_map, spatial_shape=target_amp.size()[2:]
                )
                encode_elapsed = time.time() - encode_start

                decode_start = time.time()
                latents_decoded = self.hyper_prior.decompress_forward(
                    compression_output, device=latent_map.device
                )
...
if __name__ == "__main__":
    # hyper_prior = hyperprior.Hyperprior(
    #     bottleneck_capacity=8, likelihood_type="gaussian", entropy_code=False
    # )
    # hyper_prior = None
    # net = Model(
    #     200 * 1e-2,
    ...
    #     final_phase_only=ComPhaseNet(num_in=2),
    ...
    trans_encoder = TransEncoder().cuda()
    swin_unet = SwinUnet().cuda()
    # num_down=8, num_features_init=32, max_features=256, norm=nn.BatchNorm2d
    unet = Unet(1, 1, 16, 4, 256, use_dropout=False, upsampling_mode='transpose', norm=nn.BatchNorm2d, outermost_linear=True).cuda()
    # num_in=2, latent_channels=8, num_down=2, num_up=2, norm=nn.BatchNorm2d,
    encoder = MultiScale_Encoder(per_layer_out_ch=[32, 64], in_channels=2, use_dropout=False, out_channels=8, norm=nn.BatchNorm2d).cuda()

    summary(trans_encoder, input_size=(2, 1344, 1792), batch_size=1)
    summary(swin_unet, input_size=(1, 1344, 1792), batch_size=1)
    summary(encoder, input_size=(2, 1344, 1792), batch_size=1)
    summary(unet, input_size=(1, 1344, 1792), batch_size=1)
