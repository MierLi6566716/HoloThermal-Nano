...
from src.helpers import datasets, metrics, utils
...
def phasemap_8bit(phasemap, inverted=True):
    ...
    if inverted:
        phase_out_8bit = (
            ((1 - out_phase) * 255).round().astype(np.uint8)
        )  # quantized to 8 bits
    else:
        phase_out_8bit = (
            ((out_phase) * 255).round().astype(np.uint8)
        )  # quantized to 8 bits
...
def test(test_loader, tr_agent, save_dir, postifx=""):
    ...
    for i, data in enumerate(test_loader):
        ...
        for j in range(output[0].size(0)):
            ...
            actual_bpp, theoretical_bpp = compression_utils.save_compressed_format(
                compression_output, out_path=compress_path
            )
            ...
            recon_img *= torch.sum(
                recon_img * target_amp, (-2, -1), keepdim=True
            ) / torch.sum(recon_img * recon_img, (-2, -1), keepdim=True)
            recon_img_scale *= torch.sum(
                recon_img_scale * target_amp, (-2, -1), keepdim=True
            ...
            cur_ssim_ori = ssim(
                target_amp, recon_img, multichannel=multichannel, data_range=1
            )
            cur_ssim_scale = ssim(
                target_amp, recon_img_scale, multichannel=multichannel, data_range=1
            )
            ...
            loss_all.append(
                {
                    ...
                    "mse": losses["mse"].item(),
                    "vgg": losses["vgg"].item(),
                    "wfft": losses["watson-fft"].item(),
                    "total": losses["mse"].item() + losses["vgg"].item(),
                    "psnr": cur_psnr_scale,
                    "ssim": cur_ssim_scale,
                    "actual_bpp": actual_bpp,
                    "theoretical_bpp": theoretical_bpp,
                }
            )
            ...
            errors_all.append(
                {
                    "id": index,
                    "psnr": cur_psnr_ori,
                    "psnr_scale": cur_psnr_scale,
                    "ssim": cur_ssim_ori,
                    "ssim_scale": cur_ssim_scale,
                    "actual_bpp": actual_bpp,
                    "theoretical_bpp": theoretical_bpp,
                }
            )
            print(
                "index: %s   psnr %.4f,  ssim %.4f  ac_bpp %.4f, the_bpp: %.4f"
                % (index, cur_psnr_scale, cur_ssim_scale, actual_bpp, theoretical_bpp)
            )
            ...
            scipy.io.savemat(
                str.replace(phase_name, ".png", ".mat"), {"phase": pred_phase}
            )
    ...
    for item in errors_all:
        ...
        writer.write(
            "PSNR %.4f  PSNR (scale) %.4f   SSIM %.4f  SSIM (scale) %.4f  ac_bpp: %.4f, the_bpp: %.4f \n"
            % (
                item["psnr"],
                item["psnr_scale"],
                item["ssim"],
    ...
    for item in loss_all:
        loss_writer.write(
            "id: %s total (mse + vgg):  %.4f  vgg: %.4f  mse: %.4f wfft: %.4f, ac_bpp: %.4f, the_bpp: %.4f,  psnr: %.4f  ssim: %.4f\n"
            % (
                item["id"],
                item["total"],
                item["vgg"],
...
def main():
    ...
    # model_path = f"/tmp/mufan/huaizhi/DPRC/running/DPRC/model_stage1/ckpt_epoch52.pth"
    model_path = "/tmp/mufan/huaizhi/DPRC/test_r_transformer_encoder/DPRC/model_stage2/ckpt_epoch30.pth"
    config.ckpt = model_path.split("/")[-1].split(".")[0]
    ...
    if config.model_name != "":
        save_dir = "{}/results/{}-{}-{}-ckpt-{}-compress-{}".format(
            config.exp_dir,
            mode,
            config.dataset,
            config.model_name,
        ...
        save_dir = "{}/results/{}-{}-ckpt-{}-compress-{}".format(
            config.exp_dir, mode, config.dataset, config.ckpt, config.channel
        )
    ensure_dir(save_dir)
