...
class siMSELoss(nn.Module):
    ...
    def compute_loss(self, x, label, weight=None):  ##### x.size() should be NxCxHxW
        if self.scale_invariant:
            ...
            alpha_vector = torch.div(
                denominator.sum(-1).sum(-1), numerator.sum(-1).sum(-1)
            )
            # print(alpha_vector)
...
class Grad_loss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        grad_map_input = torch.norm(
            torch.stack([input_diffx, input_diffy], dim=1), p=2, dim=1
        )
        grad_map_target = torch.norm(
            torch.stack([target_diffx, target_diffy], dim=1), p=2, dim=1
        )
        grad_map_input = (
            grad_map_input
            / torch.max(
                torch.max(grad_map_input, dim=3, keepdim=True)[0], dim=2, keepdim=True
            )[0]
        )
        grad_map_target = (
            grad_map_target
            / torch.max(
                torch.max(grad_map_target, dim=3, keepdim=True)[0], dim=2, keepdim=True
            )[0]
        )
        grad_mask = torch.lt(torch.abs(grad_map_target - 1), 0.1)

        loss = torch.mean(
            torch.mul(torch.abs(grad_map_input - grad_map_target), grad_mask),
            dim=(0, 1, 2, 3),
        )
