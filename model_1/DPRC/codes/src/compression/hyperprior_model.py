...
class HyperpriorEntropyModel(entropy_models.ContinuousEntropyModel):
    ...
    def build_tables(self, **kwargs):
        ...
        pmf = self.distribution.likelihood(
            samples.to(device), collapsed_format=True
        ).cpu()
...
class HyperpriorDensity(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for k in range(len(self.filters) + 1):
            ...
            H_k = nn.Parameter(
                torch.ones((n_channels, filters[k + 1], filters[k]))
            )  # apply softmax for non-negativity
    ...
    def lower_tail(self, tail_mass):
        ...
        lt = compression_utils.estimate_tails(
            cdf_logits_func,
            target=-np.log(2.0 / tail_mass - 1.0),
    ...
    def upper_tail(self, tail_mass):
        cdf_logits_func = lambda x: self.cdf_logits(x, update_parameters=False)
        ut = compression_utils.estimate_tails(
            cdf_logits_func,
            target=np.log(2.0 / tail_mass - 1.0),
            shape=torch.Size((self.n_channels, 1, 1)),
        ).detach()
    ...
    def median(self):
        cdf_logits_func = lambda x: self.cdf_logits(x, update_parameters=False)
        _median = compression_utils.estimate_tails(
            cdf_logits_func, target=0.0, shape=torch.Size((self.n_channels, 1, 1))
        ).detach()
...
if __name__ == "__main__":
    ...
    bits, bpp, bpi = hyperprior_entropy_model._estimate_compression_bits(
        bottleneck, spatial_shape=toy_shape[2:]
    )

    start_t = time.time()
    encoded, coding_shape, symbols = hyperprior_entropy_model.compress(
        bottleneck, block_encode=use_blocks, vectorize=vectorize
    )
    ...
    decoded, decoded_raw = hyperprior_entropy_model.decompress(
        encoded,
        n_data,
    ...
    print(
        ...
        torch.mean((decoded_raw == symbols).float()).item(),
    )
