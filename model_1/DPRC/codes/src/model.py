...
from default_config import ModelModes, ModelTypes, directories, hific_args
# Custom modules
from src import hyperprior
from src.helpers import datasets, maths, utils
from src.loss import losses
from src.loss.perceptual_similarity import perceptual_loss as ps
from src.network import discriminator, encoder, generator, hyper
...
Disc_out = namedtuple("disc_out", ["D_real", "D_gen", "D_real_logits", "D_gen_logits"])
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.use_discriminator = self.model_type == ModelTypes.COMPRESSION_GAN and (
            self.model_mode != ModelModes.EVALUATION
        )

        if self.use_discriminator is True:
            assert (
                self.args.discriminator_steps > 0
            ), "Must specify nonzero training steps for D!"
            self.discriminator_steps = self.args.discriminator_steps
            self.logger.info(
                "GAN mode enabled. Training discriminator for {} steps.".format(
    ...
    def compression_forward(self, x):
        ...
        if self.model_mode == ModelModes.EVALUATION and (self.training is False):
            n_hyperencoder_downsamples = (
                self.Hyperprior.analysis_net.n_downsampling_layers
            )
        ...
        intermediates = Intermediates(
            x, reconstruction, latents_quantized, total_nbpp, total_qbpp
        )
    ...
    def distortion_loss(self, x_gen, x_real):
        ...
        sq_err = self.squared_difference(x_gen * 255.0, x_real * 255.0)  # / 255.
    ...
    def compression_loss(self, intermediates, hyperinfo):
        ...
        weighted_rate, rate_penalty = losses.weighted_rate_loss(
            self.args,
        ...
        if self.step_counter % self.log_interval == 1:
            ...
            self.store_loss(
                "weighted_compression_loss_sans_G", weighted_compression_loss.item()
            )
    ...
    def compress(self, x, silent=False):
        ...
        * Encode latents via entropy model derived from (mean, scale) hyperprior output.
        ...
        attained_hbpp = (
            32 * len(compression_output.hyperlatents_encoded) / np.prod(spatial_shape)
        )
        attained_lbpp = (
            32 * len(compression_output.latents_encoded) / np.prod(spatial_shape)
        )
        attained_bpp = 32 * (
            (
                len(compression_output.hyperlatents_encoded)
                + len(compression_output.latents_encoded)
            )
            / np.prod(spatial_shape)
        )
    ...
    def forward(
        self, x, train_generator=False, return_intermediates=False, writeout=True
    ):
...
if __name__ == "__main__":
    ...
    logger = utils.logger_setup(
        logpath=os.path.join(directories.experiments, "logs"),
    ...
    logger.info(
        "Number of trainable parameters: {}".format(utils.count_parameters(model))
    )
    logger.info(
        "Estimated size: {} MB".format(utils.count_parameters(model) * 4.0 / 10**6)
    )
