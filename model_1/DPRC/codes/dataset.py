...
def collate_fn(batch):
    ...
    for sample in batch:
        ...
        res.append(sample[2])
        name.append(sample[3])
    imgs = torch.stack(imgs, dim=0)
    masks = torch.stack(masks, dim=0)

    return imgs, masks, res, name
...
def get_dataloader(phase, config):
    ...
    if config.dataset == "DIV2K":
        if phase == "train":
            data_path = os.path.join(
                config.holo_data_root, f"{config.dataset}_train_HR"
            )
        else:
            data_path = os.path.join(
                config.holo_data_root, f"{config.dataset}_valid_HR"
            )
    else:
    ...
    dataset = ImageLoader(
        ...
        shuffle=is_shuffle,
        vertical_flips=is_vertical,
        horizontal_flips=is_horizontal,
        fake=config.fake,
    )
    dataloader = DataLoader(
        dataset, num_workers=8, shuffle=False, batch_size=1, collate_fn=collate_fn
    )
...
class ImageLoader:
    ...
    def __init__(
        ...
        print(data_path)
        if not os.path.isdir(data_path) and not fake:
        ...
        if not fake:
            ...
            self.im_names = [f"fake_image_{i}.png" for i in range(number_of_images)]
    ...
    def __getitem__(self, index):
        batch_ims = self.order[index]
        datas = self.load_image(*batch_ims)
    ...
    def load_image(self, filenum, *augmentation_states):
        if not self.fake:
            ...
            im = np.random.rand(self.image_res[0], self.image_res[1], 3)
...
class FakeImageLoader:
    def __init__(
        ...
        # Simulate a list of image names
        self.im_names = [f"fake_image_{i}.png" for i in range(number_of_images)]
    ...
    def __next__(self):
        ...
        if self.ind < len(self.order):
            batch_indices = self.order[self.ind : self.ind + self.batch_size]
            self.ind += self.batch_size
            return self.load_batch(batch_indices)
        else:
            raise StopIteration

    def __getitem__(self, index):
        return self.load_image(self.order[index])
    ...
    def load_batch(self, indices):
        im_res_name = [self.load_image(i) for i in indices]
        ims = torch.stack([im for im, _, _, _ in im_res_name], 0)
        masks = torch.stack([mask for _, mask, _, _ in im_res_name], 0)
        return (
            ims,
    ...
    def load_image(self, index):
        ...
        channels = 3 if self.channel is None else 1
        fake_im = torch.rand(channels, *self.image_res)
        fake_mask = torch.ones_like(fake_im)

        # Simulate original image resolution
        input_res = (random.randint(500, 2000), random.randint(500, 2000))

        filename = os.path.splitext(self.im_names[index])[0]

        return fake_im, fake_mask, input_res, filename
