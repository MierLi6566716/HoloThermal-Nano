...
def calc_ssim(img1, img2, datarange=255.0):
    im1 = img1.numpy().transpose(1, 2, 0).astype(np.uint8)
    im2 = img2.numpy().transpose(1, 2, 0).astype(np.uint8)
    return compare_ssim(
        im1, im2, datarange=datarange, multichannel=True, gaussian_weights=True
    )


def calc_metrics(im_pred, im_gt, mask=None):
    q_im_pred = quantize(im_pred.data, rgb_range=1.0)
    q_im_gt = quantize(im_gt.data, rgb_range=1.0)
    if mask is not None:
...
def save_image(img, path):
    ...
    if len(img.size()) == 2:  # grayscale image
        ...
        im = Image.fromarray(
            q_im.permute(1, 2, 0).cpu().numpy().astype(np.uint8), "RGB"
        )
    else:
        pass
    im.save(path)
...
def save_batch_images(output, imgpath, save_dir, alpha=0.5):
    ...
    q_im_output = [quantize(o.data, rgb_range=1.0) for o in output]
        ...
        for o in range(len(output)):
            if o % 2 == 1 or len(output) == 1:
                output_img = Image.fromarray(
                    q_im_output[o][b].permute(1, 2, 0).cpu().numpy().astype(np.uint8),
                    "RGB",
                )
                if GEN:
...
def save_batch_images_test(output, imgpath, save_dir, alpha=0.5):
    ...
    for b in range(output[0].size(0)):
        ...
        for o in range(len(output)):
            ...
            output_img = Image.fromarray(
                q_im_output[o][b].permute(1, 2, 0).cpu().numpy().astype(np.uint8), "RGB"
            )
            if GEN:
...
def save_images_test(output, imgpath, save_dir, alpha=0.5):
    ...
    for b in range(output[0].size(0)):
        ...
        output_img = Image.fromarray(
            q_im_output[0][b].permute(1, 2, 0).cpu().numpy().astype(np.uint8), "RGB"
        )
...
def make_video(out_dir, gt_dir, gt_first=False):
    gt_ext = "/*.png"
    frames_all = sorted(
        glob.glob(out_dir + "/*.png") + glob.glob(gt_dir + gt_ext),
