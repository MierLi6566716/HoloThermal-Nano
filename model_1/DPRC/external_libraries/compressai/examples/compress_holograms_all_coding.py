...
def find_closest_msssim(target, img, fmt="jpeg"):
    ...
    def _mssim(a, b):
        a = (
            torch.from_numpy(np.asarray(a).astype(np.float32))
            .permute(2, 0, 1)
            .unsqueeze(0)
        )
        b = (
            torch.from_numpy(np.asarray(b).astype(np.float32))
            .permute(2, 0, 1)
            .unsqueeze(0)
        )
...
root_dir = (
    "/jixie/yujie_codes/holonet_compression/our_final_results_0821/all_phase_data/"
)
...
for key, value in model_dict.items():
    ...
    for quality in quality_list:
        ...
        writer = open(
            os.path.join(result_dir, f"quantitatives-quality{quality}.txt"), "a"
        )
        writer.write(
            "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n"
        )
        ...
        print(
            f"Entropy bottleneck(s) parameters: {sum(p.numel() for p in net.aux_parameters())}"
        )
            ...
            for sub_dir in [
                ...
                if "color" not in sub_dir:
                    ...
                    img_path = os.path.join(
                        root_dir, sub_dir, f"{target_idx:04d}-pred-phase-rgb.png"
                    )
                ...
                with torch.no_grad():
                    ...
                    if key == "hyperprior18":
                        ...
                        decompress_out = net.decompress(
                            compress_out["strings"], compress_out["shape"]
                        )
                    ...
                    writer.write(
                        f"PSNR: {psnr:.4f}\t MS-SSIM: {ssim:.4f} \t  Bit-rate: {target_bpp:.4f} \n"
                    )
                    ...
                    save_name = os.path.join(
                        result_dir,
                        f"{target_idx:04d}-pred-phase-{key}-quality{quality}.png",
                    )
