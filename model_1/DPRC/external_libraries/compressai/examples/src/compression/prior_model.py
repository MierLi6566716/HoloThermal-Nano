...
from src.compression import compression_utils, entropy_coding, entropy_models
# Custom
...
def prior_scale_table(
    scales_min=SCALES_MIN, scales_max=SCALES_MAX, levels=SCALES_LEVELS
):
...
class PriorEntropyModel(entropy_models.ContinuousEntropyModel):
    ...
    def __init__(
        ...
        index_ranges=64,
        min_scale=MIN_SCALE,
        likelihood_bound=MIN_LIKELIHOOD,
        tail_mass=TAIL_MASS,
    ...
    def build_tables(self, **kwargs):
        ...
        upper = self.standardized_CDF((0.5 - samples) / samples_scale)
        lower = self.standardized_CDF((-0.5 - samples) / samples_scale)
        ...
        for n, (pmf_, pmf_length_, tail_) in enumerate(
            (zip(tqdm(pmf), pmf_length, tail_mass))
        ):
            pmf_ = pmf_[:pmf_length_]  # [max_length]
            overflow = torch.clamp(1.0 - torch.sum(pmf_, dim=0, keepdim=True), min=0.0)
    ...
    def compute_indices(self, scales):
        ...
        indices = torch.ones_like(scales, dtype=torch.int32) * (
            len(self.scale_table) - 1
        )
    ...
    def compress(
        ...
        assert (
            symbols.size() == indices.size()
        ), "Indices should have same size as inputs."
    ...
    def decompress(
        ...
        decoded = self.decoder.decode_with_indexes(
            encoded, indices, cdf, cdf_length, cdf_offset
        )
        #
        ...
        print(
            "in prior model, after decoding, transfer time is ",
...
class PriorDensity(nn.Module):
    ...
    def upper_tail(self, tail_mass, mean, scale):
        tail_mass = float(tail_mass)
        ut = self.quantile(1.0 - 0.5 * tail_mass, mean=mean, scale=scale)
        return ut
...
if __name__ == "__main__":
    ...
    bits, bpp, bpi = prior_entropy_model._estimate_compression_bits(
        bottleneck, means, scales, spatial_shape=toy_shape[2:]
    )

    start_t = time.time()

    encoded, coding_shape, rounded = prior_entropy_model.compress(
        bottleneck, means, scales, block_encode=use_blocks, vectorize=vectorize
    )
    ...
    decoded, decoded_raw = prior_entropy_model.decompress(
        encoded,
        means,
        scales,
        broadcast_shape=toy_shape[2:],
    ...
    print(
        f"Delta t {delta_t:.2f} s | ",
        torch.mean((decoded_raw == rounded).float()).item(),
    )
