...
def setup_generic_signature(args, special_info):
    ...
    time_signature = "{:%Y_%m_%d_%H:%M}".format(datetime.datetime.now()).replace(
        ":", "_"
    )
    if args.name is not None:
        args.name = "{}_{}_{}_{}".format(
            args.name, args.dataset, special_info, time_signature
        )
    else:
...
def save_model(
    model, optimizers, mean_epoch_loss, epoch, device, args, logger, multigpu=False
):
    ...
    metadata_path = os.path.join(
        directory, "metadata/model_{}_metadata_{}.json".format(model_name, timestamp)
    )
    ...
    model_path = os.path.join(
        directory,
        "{}_epoch{}_idx{}_{}.pt".format(
            model_name, epoch, model.step_counter, timestamp
        ),
    )
    ...
    if os.path.exists(model_path):
        model_path = os.path.join(
            directory,
            "{}_epoch{}_idx{}_{:%Y_%m_%d_%H:%M:%S}.pt".format(
                model_name, epoch, model.step_counter, datetime.datetime.now()
            ),
        )

    save_dict = {
        "model_state_dict": (
            model.module.state_dict() if args.multigpu is True else model.state_dict()
        ),
    ...
    if model.use_discriminator is True:
        save_dict["discriminator_state_dict"] = (
            model.module.Discriminator.state_dict()
            if args.multigpu is True
            else model.Discriminator.state_dict()
        )
        save_dict["discriminator_optimizer_state_dict"] = optimizers[
            "disc"
        ].state_dict()

    torch.save(save_dict, f=model_path)
    logger.info(
        "Saved model at Epoch {}, step {} to {}".format(
            epoch, model.step_counter, model_path
        )
    )
...
def load_model(
        ...
        if silent is False:
            for k, v in current_args_d.items():
                try:
                    ...
                    logger.warning(
                        "Argument {} (value {}) not present in recorded arguments. Using current argument.".format(
                            k, v
                        )
                    )
                    continue
    ...
    if silent is False:
        ...
        logger.info(
            "Number of trainable parameters: {}".format(count_parameters(model))
        )
    logger.info(
        "Estimated model size (under fp32): {:.3f} MB".format(
            count_parameters(model) * 4.0 / 10**6
        )
    )
    ...
    if prediction is True:
        ...
        hyperlatent_likelihood_parameters = (
            model.Hyperprior.hyperlatent_likelihood.parameters()
        )

        amortization_opt = torch.optim.Adam(
            amortization_parameters, lr=args.learning_rate
        )
        hyperlatent_likelihood_opt = torch.optim.Adam(
            hyperlatent_likelihood_parameters, lr=args.learning_rate
        )
        ...
        if args.sample_noise is True:
            optimizers["amort"].add_param_group(
                {"params": list(model.Generator.latent_noise_map.parameters())}
            )

        optimizers["amort"].load_state_dict(
            checkpoint["compression_optimizer_state_dict"]
        )
        optimizers["hyper"].load_state_dict(
            checkpoint["hyperprior_optimizer_state_dict"]
        )
        if (model.use_discriminator is True) and ("disc" in optimizers.keys()):
            try:
                optimizers["disc"].load_state_dict(
...
def log_summaries(writer, storage, step, use_discriminator=False):
    ...
    weighted_compression_scalars = [
        "weighted_compression_loss",
        "weighted_R_D",
        "weighted_rate",
        "weighted_distortion",
        "weighted_perceptual",
    ...
    compression_loss_breakdown = dict(
        total_comp=storage["weighted_compression_loss"][-1],
    ...
    for scalar in weighted_compression_scalars:
        writer.add_scalar(
            "weighted_compression/{}".format(scalar), storage[scalar][-1], step
        )
    ...
    if use_discriminator is True:
        compression_loss_breakdown["weighted_gen_loss"] = storage["weighted_gen_loss"][
            -1
        ]
    ...
    **kwargs,
    ...
    if writer is not None:
        log_summaries(
            ...
            storage,
            model.step_counter,
            use_discriminator=model.use_discriminator,
        )
    ...
    if header == "[TRAIN]":
        ...
        report_f(
            "Epoch {} | Mean epoch comp. loss: {:.3f} | Current comp. loss: {:.3f} | "
            "Rate: {} examples/s | Time: {:.1f} s | Improved: {}".format(
                epoch,
                mean_epoch_loss,
                current_loss,
                int(batch_size * idx / ((time.time() - t0))),
        ...
        report_f(
            "Epoch {} | Mean epoch comp. loss: {:.3f} | Current comp. loss: {:.3f} | Improved: {}".format(
                epoch, mean_epoch_loss, current_loss, improved
            )
        )
    report_f("========>")
    ...
    report_f(
        ...
        "n_bpp (hyp-latent): {:.3f} | q_bpp (hyp-latent): {:.3f}".format(
            avg_bpp,
            storage["n_rate"][-1],
            storage["q_rate"][-1],
    ...
    if model.use_discriminator is True:
        ...
        report_f(
            "G Loss: {:.3f} | D Loss: {:.3f} | D(gen): {:.3f} | D(real): {:.3f}".format(
                storage["gen_loss"][-1],
                storage["disc_loss"][-1],
                storage["D_gen"][-1],
                storage["D_real"][-1],
