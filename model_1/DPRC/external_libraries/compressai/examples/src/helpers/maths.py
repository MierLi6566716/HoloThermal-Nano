...
def pmf_to_quantized_cdf(pmf, precision, careful=True):
    ...
    assert torch.all(pmf >= 0.0).item(), "PMF must have all non-negative values"
...
class LowerBoundToward(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output):
        gate = torch.logical_or(ctx.mask, grad_output.lt(0.0)).type(grad_output.dtype)
...
def standardized_CDF_gaussian(value):
    ...
    return 0.5 * torch.erfc(value * (-1.0 / np.sqrt(2)))
...
def kl_divergence_q_prior_normal(mu, logvar):
    ...
    ), "Mean and log-variance must share shape (batch, latent_dim)"
