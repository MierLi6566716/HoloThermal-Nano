...
class L2(FakeNet):
    ...
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if self.colorspace == "RGB":
            ...
            value = pl.l2(
                pl.tensor2np(pl.tensor2tensorlab(in0.data, to_norm=False)),
                pl.tensor2np(pl.tensor2tensorlab(in1.data, to_norm=False)),
                range=100.0,
            ).astype("float")
...
class DSSIM(FakeNet):
    ...
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if self.colorspace == "RGB":
            value = pl.dssim(
                1.0 * pl.tensor2im(in0.data), 1.0 * pl.tensor2im(in1.data), range=255.0
            ).astype("float")
        elif self.colorspace == "Lab":
            value = pl.dssim(
                pl.tensor2np(pl.tensor2tensorlab(in0.data, to_norm=False)),
